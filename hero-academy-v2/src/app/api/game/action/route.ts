...
// rollArtifactDrop is now imported from @/lib/game/constants

/* ── Classification by `effect` field (DB maps all shields to 'damage_reduce' in effect_type) ──
 * Full shield: effect='damage_shield' AND effect_value >= 100
 * Passive %:   effect='dmg_reduce' OR 'passive_damage_reduction' OR 'damage_shield' val<100
 */
function classifyDmgArtifactA(eff: string, val: number): 'full_shield' | 'passive_reduce' | 'none' {
...
/* ── Decrement 1 charge, delete if exhausted ── */
async function decrementChargeA(heroArtifactId: string, currentCharges: number) {
...
async function getArtifactModifiers(heroId: string, actionType: 'damage' | 'grant_xp' | 'grant_gold') {
  ...
  const toDecrement: string[] = [];  // non-damage charge-based artifacts to decrement
  const passiveDmgArts: PassiveDmgArtA[] = [];
  let shield: { heroArtifactId: string; name: string; chargesLeft: number } | null = null;
  ...
  if (artifacts) {
    ...
    for (const a of artifacts) {
      ...
      // Classify damage artifacts BEFORE the loop (once per artifact, not per sub-effect)
      const dmgClass = classifyDmgArtifactA(effect, effectValue);

      // ── Full Shield (damage_shield with value >= 100) ── OUTSIDE loop
      if (actionType === 'damage' && dmgClass === 'full_shield' && !shield) {
        shield = { heroArtifactId: String(rec.id), name: String(art.name), chargesLeft };
        contributed = true;
      }

      // ── Passive dmg reduction ── OUTSIDE loop
      ...
      // XP/gold/boss effects still iterate sub-effects (for combo like 'xp_boost,gold_boost')
      ...
      // Track for labels (exclude shield from applied list)
      if (contributed && shield?.heroArtifactId !== String(rec.id)) {
        ...
        // Charge decrement for non-damage artifacts (xp, gold boosts)
  ...
  for (const haId of toDecrement) {
    ...
    if (ha) {
      await decrementChargeA(haId, ha.charges_remaining as number);
    }
  }
  ...
  try {
    ...
    if (action === 'damage') {
      ...
      if (arts.shield) {
        ...
        await decrementChargeA(arts.shield.heroArtifactId, arts.shield.chargesLeft);
        ...
        for (const art of arts.passiveDmgArts) {
          if (art.maxCharges > 0) {
            await decrementChargeA(art.heroArtifactId, art.chargesLeft);
          }
        }
      } else {
        // ── No protection ──
      ...
      // DEAL DAMAGE TO SUBJECT BOSS
      if (subject) {
        // Find user class & active season
          ...
          if (classData && classData.class_id && classData.school_id) {
            ...
            if (season) {
               ...
               if (boss && boss.current_hp > 0) {
                 // Apply boss damage boost from artifacts (e.g. Коготь Дракона +20%)
                 ...
                 const newBossHp = Math.max(0, boss.current_hp - bossDamage);
                 ...
                 pipeline.push(`⚔️ Босс '${subject}' получил -${bossDamage} HP`);

                 // BOSS DEFEAT MECHANIC — delegated to shared function
                 if (newBossHp === 0 && boss.current_hp > 0) {
