...
export async function POST(req: NextRequest) {
  ...
  // Verify this lootbox belongs to someone
  const { data: boxRow } = await admin.from('hero_artifacts').select('id, hero_id').eq('id', heroArtifactId).single();
  if (!boxRow) return NextResponse.json({ error: 'Lootbox not found in inventory' }, { status: 404 });

  // ============================================================
  // SEASONAL LOOTBOX PATH
  // If boxRarity is a seasonal tag (e.g. 'seasonal_fire'),
  // skip rarity weights entirely and roll from the season pool.
  // ============================================================
  const seasonPool = SEASONAL_POOLS[boxRarity];
  if (seasonPool) {
  ...
  // Build pool: correct rarity, not a lootbox, within level cap, NOT seasonal
  const { data: pool } = await admin
    .from('artifacts')
    .select('id, name, icon, rarity, drop_rate, min_level')
    .eq('rarity', chosenTier)
    .neq('effect', 'lootbox')
    .is('season_pool', null)    // exclude seasonal artifacts from standard pool
    .lte('min_level', heroLevel + 5);
