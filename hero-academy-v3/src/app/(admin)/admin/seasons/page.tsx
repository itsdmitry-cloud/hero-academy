...
export default function SeasonsPage() {
  ...
  const handleCreate = async () => {
    ...
    if (!schoolId) { setFeedback('Нет школ — создайте школу первым'); return; }
    setSaving(true);
    ...
    <div className={styles.page}>
      ...
      {showAdd && (
        <div style={{ background: 'var(--bg-glass)', border: '1px solid var(--bg-glass-border)', borderRadius: 'var(--radius-xl)', padding: '1.25rem', marginBottom: '1.5rem', display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.8rem', marginBottom: '0.25rem', color: 'var(--text-secondary)' }}>Конец *</label>
          ...
          <button onClick={handleCreate} disabled={saving || !name.trim() || !startDate || !endDate} style={{ padding: '0.6rem 1.5rem', background: 'var(--accent-primary)', color: 'white', border: 'none', borderRadius: 'var(--radius-lg)', fontWeight: 700, cursor: 'pointer' }}>
            {saving ? '⏳' : 'Создать'}
          </button>
        </div>
      )}
