...
import type { TeacherInitialData, LiveInitialData } from '@/lib/teacher/types';
...
interface Props {
  initialTeacherData: TeacherInitialData;
  initialLiveData: LiveInitialData | null;
}

export default function LiveRadarClient({ initialTeacherData, initialLiveData }: Props) {
  useAuth();
  const { activeClassId, subjects, grantXp, damageHp, activeSubject, createQuest, quests } = useTeacherData(initialTeacherData);
  const { students: liveStudents, loading, optimisticUpdate } = useRealtimeClass(
    activeClassId,
    initialLiveData?.liveStudents,
  );
  ...
  const [allLessonCounters, setAllLessonCounters] = useState<Record<string, Record<string, Record<string, number>>>>(initialLiveData?.counters ?? {});

  // SSR уже залил counters для активного класса. Клиент перезапрашивает только
  // когда класс меняется (после первого рендера) или когда SSR-снимка не было.
  const skipFirstCountersFetch = useRef(!!initialLiveData && activeClassId === initialTeacherData.activeClassId);
  useEffect(() => {
    if (!activeClassId) return;
    if (skipFirstCountersFetch.current) {
      skipFirstCountersFetch.current = false;
      return;
    }
    const load = async () => {
  ...
  const [allAverages, setAllAverages] = useState<Record<string, Record<string, number>>>(initialLiveData?.averages ?? {});

  // Refetch averages whenever students change (class switch / новый ученик).
  // Первый рендер с SSR-снимком пропускаем — данные уже в state.
  const skipFirstAveragesFetch = useRef(!!initialLiveData);
  useEffect(() => {
    if (skipFirstAveragesFetch.current) {
      skipFirstAveragesFetch.current = false;
      return;
    }
    let cancelled = false;
    (async () => {
