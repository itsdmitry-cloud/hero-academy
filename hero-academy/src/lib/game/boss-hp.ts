 ...
 *
 * Формула:
 *   hp = students × lessons_per_week × weeks × avg_dmg_per_lesson
 *
 ...
 * урона. Умножаем на длительность сезона и размер класса.
 *
 ...
 *
 * Минимальный порог HP = 1000, чтобы даже в пустом классе с коротким
 * сезоном босс не становился «ваткой» (иначе ученик одним рывком его добьёт
 * и механика теряет смысл).
 */
...
export function calculateBossHp({ studentCount, seasonWeeks }: BossHpInput): number {
  ...
  const raw = students * LESSONS_PER_WEEK * weeks * AVG_DAMAGE_PER_LESSON;
