...
export const ARTIFACT_REGISTRY: ArtifactEntry[] = [
  ...
  art('com_shield',    'Деревянный Щит',         '-10% урон (3 заряда)',                      'common', '🛡️', 'passive',    'DMG_REDUCE_10',    { max_charges: 3, is_shopable: true }),
  ...
  art('com_scroll',    'Свиток Концентрации',    '-50% урон (1 заряд)',                       'common', '📃', 'passive',    'DMG_REDUCE_50',    { max_charges: 1, req_level: 2 }),
  art('com_ring',      'Бронзовое Кольцо',       'Gold +5% постоянно',                       'common', '💍', 'passive',    'GOLD_BOOST_5',     { req_level: 2 }),
  art('com_ink',       'Флакон Чернил',          '-5 урона (5 зарядов)',                      'common', '🖋️', 'passive',    'FLAT_DMG_REDUCE_5',{ max_charges: 5, req_level: 3 }),
  ...
  art('rar_armor',     'Броня Усидчивости',      '-30% урон (5 зарядов)',                     'rare', '🛡️', 'passive',    'DMG_REDUCE_30',     { max_charges: 5, req_level: 5, is_shopable: true }),
  art('rar_pouch',     'Кошель Удачи',           'Gold +30% на 48 часов',                    'rare', '👛', 'passive',    'GOLD_BOOST_30',     { duration_hours: 48, req_level: 5 }),
  art('rar_candle',    'Свеча Полуночника',      'Защита стрика (1 раз)',                     'rare', '🕯️', 'passive',    'PROTECT_STREAK',    { max_charges: 1, req_level: 5, is_shopable: true }),
  ...
  art('rar_shield',    'Щит Стражника',          '-50% урон (2 заряда)',                      'rare', '🛡️', 'passive',    'DMG_REDUCE_50',     { max_charges: 2, req_level: 8 }),
  art('rar_focus',     'Зелье Фокуса',           '+100 XP моментально',                      'rare', '🧪', 'consumable', 'FLAT_XP_100',       { req_level: 8 }),
  art('rar_cloak',     'Плащ Ветра',             'Полная блокировка 1 ошибки',                'rare', '🪭', 'passive',    'BLOCK_ONE_MISTAKE', { max_charges: 1, req_level: 9 }),
  art('rar_elixir',    'Эликсир Озарения',       'XP +50% на 5 часов',                       'rare', '⚗️', 'passive',    'XP_BOOST_50',       { duration_hours: 5, req_level: 9 }),

  // ── 🟣 EPIC (10) ──
  art('epi_orb',       'Сфера Архимага',         'XP +50% за урок (3 заряда)',                'epic', '🔮', 'passive',    'CLASSWORK_XP_50',   { max_charges: 3, req_level: 15 }),
  art('epi_shield',    'Мифриловый Щит',         'Полная блокировка крит. урона (2 заряда)',   'epic', '🛡️', 'passive',    'BLOCK_CRITICAL_DMG',{ max_charges: 2, req_level: 15 }),
  ...
  art('epi_armor',     'Адамантитовый Нагрудник', '-70% урон (3 заряда)',                      'epic', '🦺', 'passive',    'DMG_REDUCE_70',     { max_charges: 3, req_level: 18 }),
  art('epi_crystal',   'Кристалл Охотника',      '+200 XP за босса (1 заряд)',                'epic', '💎', 'passive',    'FLAT_BOSS_XP_200',  { max_charges: 1, req_level: 19 }),
  ...
  // ── 🟡 LEGENDARY (10) ──
  art('leg_crown',     'Корона Академии',        'XP +100%, Gold +50% на неделю',             'legendary', '👑', 'passive',    'XP_GOLD_MASSIVE',   { duration_hours: 168, req_level: 25 }),
  ...
  art('leg_staff',     'Посох Властителя',       '+200 XP за каждый урок (5 зарядов)',        'legendary', '🏑', 'passive',    'CLASSWORK_XP_200',  { max_charges: 5, req_level: 27 }),
  art('leg_dragon',    'Золотой Дракон',         'Gold ×3 на неделю',                         'legendary', '🐉', 'passive',    'GOLD_MULTIPLIER_3X',{ duration_hours: 168, req_level: 28 }),
  art('leg_aegis',     'Непробиваемая Эгида',    'Все ошибки блокированы (3 заряда)',          'legendary', '🛡️', 'passive',    'BLOCK_ALL_MISTAKES',{ max_charges: 3, req_level: 29 }),
  art('leg_elixir',    'Эликсир Гения',          'Мгновенный уровень',                        'legendary', '🧪', 'consumable', 'FORCE_LEVEL_UP',    { req_level: 30 }),
  art('leg_ringall',   'Кольцо Всевластия',      '+10% XP всему классу на неделю',            'legendary', '💍', 'passive',    'TEAM_XP_10',        { duration_hours: 168, req_level: 32 }),
  art('leg_scroll',    'Свиток Истины',           'XP за босса ×3 (1 заряд)',                  'legendary', '📜', 'passive',    'BOSS_MULTIPLIER_3X',{ max_charges: 1, req_level: 35 }),
  art('leg_star',      'Звезда Академии',        'Бесконечный стрик на месяц',                'legendary', '⭐', 'passive',    'INFINITE_STREAK',   { duration_hours: 720, req_level: 40 }),
  ...
  art('fire_dragon_scale',     'Драконья Чешуйка',   'Защита стрика (1 раз)',           'common', '🐉', 'passive',    'FIRE_STREAK_SHIELD',   { max_charges: 1, season_tag: 'fire' }),
  art('fire_phoenix_feather',  'Огненное Перо',      'Авто-воскрешение с 50 HP',        'common', '🪶', 'passive',    'FIRE_AUTO_RESURRECT',  { max_charges: 1, season_tag: 'fire' }),
  ...
  art('fire_team_boss_potion', 'Огненный Натиск',    '+20% урона боссу (весь класс, 24ч)', 'rare',   '🔥', 'passive',    'TEAM_BOSS_20',         { duration_hours: 24, season_tag: 'fire' }),
];
