...
function mapEffectToDB(code: string): { effect_type: EffectTypeEnum; effect: string; effect_value: number } {
  ...
  if (code === 'FLAT_XP_100')         return { effect_type: 'xp_boost',       effect: 'xp_boost',              effect_value: 100 };
  // Special
  ...
  if (code === 'FIRE_XP_50')          return { effect_type: 'xp_boost',       effect: 'xp_boost',              effect_value: 50 };
  ...
  if (code === 'FIRE_XP_200')         return { effect_type: 'xp_boost',       effect: 'xp_boost',              effect_value: 200 };
  if (code === 'FIRE_BOSS_500')       return { effect_type: 'xp_boost',       effect: 'boss_dmg',              effect_value: 500 };
  if (code === 'FIRE_SEASON_XP_50')   return { effect_type: 'xp_boost',       effect: 'xp_boost',              effect_value: 50 };
  if (code === 'FIRE_RANDOM_GOLD')    return { effect_type: 'gold_bonus',     effect: 'extra_gold',            effect_value: 50 };
  if (code === 'FIRE_COMBO')          return { effect_type: 'xp_boost',       effect: 'xp_boost,gold_boost',   effect_value: 30 };
  if (code === 'FIRE_GOLD_MULT_10')   return { effect_type: 'gold_bonus',     effect: 'gold_boost',            effect_value: 10 };
  if (code === 'FIRE_BOSS_RANDOM')    return { effect_type: 'xp_boost',       effect: 'boss_dmg',              effect_value: 100 };
  ...
  if (code === 'FIRE_CLASS_BOSS_3000') return { effect_type: 'xp_boost',      effect: 'boss_dmg',              effect_value: 3000 };
  if (code === 'FIRE_CLASS_HP_20')    return { effect_type: 'hp_shield',      effect: 'hp_restore',            effect_value: 20 };
  if (code === 'FIRE_CLASS_XP_100')   return { effect_type: 'xp_boost',       effect: 'xp_boost',              effect_value: 100 };
  if (code === 'FIRE_CLASS_GOLD_50')  return { effect_type: 'gold_bonus',     effect: 'extra_gold',            effect_value: 50 };
  if (code === 'FIRE_RANDOM_STUDENT') return { effect_type: 'xp_boost',       effect: 'xp_boost',              effect_value: 50 };
...
export const ARTIFACT_REGISTRY: ArtifactEntry[] = [
  ...
  // ── 🟢 COMMON (10) ──
  art('com_potion',    'Малое Снадобье Памяти',  'Восстановить 30 HP',                       'common', '🧪', 'consumable', 'HEAL_30',          { is_shopable: true, drop_rate: 0.25 }),
  art('com_pen',       'Ученическое Перо',       'XP +10%',                                  'common', '🪶', 'passive',    'XP_BOOST_10',      { duration_hours: 24, is_shopable: true }),
  art('com_shield',    'Деревянный Щит',         '-10% к урону',                              'common', '🛡️', 'passive',    'DMG_REDUCE_10',    { max_charges: 3, is_shopable: true }),
  art('com_parchment', 'Рваный Пергамент',       'Gold +10%',                                'common', '📜', 'passive',    'GOLD_BOOST_10',    { duration_hours: 24, is_shopable: true }),
  art('com_coin',      'Медная Монета',          '+5 Gold за квест',                         'common', '🪙', 'passive',    'FLAT_GOLD_5',      { max_charges: 3, is_shopable: true }),
  art('com_scroll',    'Свиток Концентрации',    '-50% к урону',                              'common', '📃', 'passive',    'DMG_REDUCE_50',    { max_charges: 1, req_level: 2 }),
  art('com_ring',      'Бронзовое Кольцо',       'Gold +5%',                                 'common', '💍', 'passive',    'GOLD_BOOST_5',     { req_level: 2 }),
  art('com_ink',       'Флакон Чернил',          '-5 к урону',                                'common', '🖋️', 'passive',    'FLAT_DMG_REDUCE_5',{ max_charges: 5, req_level: 3 }),
  art('com_cloak',     'Плащ Новичка',           'XP +5%, Gold +5%',                         'common', '🧥', 'passive',    'XP_GOLD_5',        { duration_hours: 12, req_level: 3 }),
  art('com_magnet',    'Магнит Жадности',        'Gold +20%',                                'common', '🧲', 'passive',    'GOLD_BOOST_20',    { duration_hours: 6, req_level: 4 }),

  // ── 🔵 RARE (10) ──
  art('rar_potion',    'Среднее Зелье Бодрости', 'Восстановить 60 HP',                       'rare', '🧪', 'consumable', 'HEAL_60',           { req_level: 5, is_shopable: true }),
  art('rar_armor',     'Броня Усидчивости',      '-30% к урону',                              'rare', '🛡️', 'passive',    'DMG_REDUCE_30',     { max_charges: 5, req_level: 5, is_shopable: true }),
  art('rar_pouch',     'Кошель Удачи',           'Gold +30%',                                'rare', '👛', 'passive',    'GOLD_BOOST_30',     { duration_hours: 48, req_level: 5 }),
  art('rar_candle',    'Свеча Полуночника',      'Защита стрика',                             'rare', '🕯️', 'passive',    'PROTECT_STREAK',    { max_charges: 1, req_level: 5, is_shopable: true }),
  art('rar_pen',       'Перо Калиграфа',         'XP +20%',                                  'rare', '✒️',  'passive',    'XP_BOOST_20',       { duration_hours: 24, req_level: 6 }),
  art('rar_amulet',    'Серебряный Амулет',      'XP +15%, Gold +15%',                       'rare', '📿', 'passive',    'XP_GOLD_15',        { duration_hours: 48, req_level: 7 }),
  art('rar_shield',    'Щит Стражника',          '-50% к урону',                              'rare', '🛡️', 'passive',    'DMG_REDUCE_50',     { max_charges: 2, req_level: 8 }),
  art('rar_focus',     'Зелье Фокуса',           '+100 XP',                                  'rare', '🧪', 'consumable', 'FLAT_XP_100',       { req_level: 8 }),
  art('rar_cloak',     'Плащ Ветра',             'Полная блокировка ошибки',                  'rare', '🪭', 'passive',    'BLOCK_ONE_MISTAKE', { max_charges: 1, req_level: 9 }),
  art('rar_elixir',    'Эликсир Озарения',       'XP +50%',                                  'rare', '⚗️', 'passive',    'XP_BOOST_50',       { duration_hours: 5, req_level: 9 }),

  // ── 🟣 EPIC (10) ──
  art('epi_orb',       'Сфера Архимага',         'XP +50% за урок',                           'epic', '🔮', 'passive',    'CLASSWORK_XP_50',   { max_charges: 3, req_level: 15 }),
  art('epi_shield',    'Мифриловый Щит',         'Полная блокировка крит. урона',              'epic', '🛡️', 'passive',    'BLOCK_CRITICAL_DMG',{ max_charges: 2, req_level: 15 }),
  art('epi_scroll',    'Свиток Выходного Дня',   'Пропустить ДЗ без потери HP',               'epic', '📜', 'consumable', 'SKIP_HOMEWORK',     { req_level: 15 }),
  art('epi_potion',    'Большое Зелье',          'Полное восстановление HP',                  'epic', '🧪', 'consumable', 'HEAL_100',          { req_level: 15, is_shopable: true }),
  art('epi_cup',       'Золотая Чаша',           'Gold +100%',                                'epic', '🏆', 'passive',    'GOLD_BOOST_100',    { duration_hours: 48, req_level: 16 }),
  art('epi_rune',      'Руна Знаний',            'XP +50%',                                  'epic', '🔷', 'passive',    'XP_BOOST_50',       { duration_hours: 48, req_level: 17 }),
  art('epi_armor',     'Адамантитовый Нагрудник', '-70% к урону',                              'epic', '🦺', 'passive',    'DMG_REDUCE_70',     { max_charges: 3, req_level: 18 }),
  art('epi_crystal',   'Кристалл Охотника',      '+200 XP за босса',                          'epic', '💎', 'passive',    'FLAT_BOSS_XP_200',  { max_charges: 1, req_level: 19 }),
  art('epi_ring',      'Кольцо Алхимика',        'XP +50%, Gold +50%',                       'epic', '💍', 'passive',    'XP_GOLD_50',        { duration_hours: 24, req_level: 20 }),
  art('epi_feather',   'Младшее Перо Феникса',   'Спасение от смерти (30 HP)',                'epic', '🪶', 'passive',    'PREVENT_DEATH_30',  { max_charges: 1, req_level: 20 }),

  // ── 🟡 LEGENDARY (10) ──
  art('leg_crown',     'Корона Академии',        'XP +100%, Gold +50%',                       'legendary', '👑', 'passive',    'XP_GOLD_MASSIVE',   { duration_hours: 168, req_level: 25 }),
  art('leg_hourglass', 'Песочные Часы Времени',  'Повтор квеста',                             'legendary', '⏳', 'consumable', 'RETRY_QUEST',       { req_level: 25 }),
  art('leg_cross',     'Крест Возрождения',      'Спасение от смерти (50 HP)',                'legendary', '✝️',  'passive',    'PREVENT_DEATH_50',  { max_charges: 1, req_level: 26 }),
  art('leg_staff',     'Посох Властителя',       '+200 XP за каждый урок',                    'legendary', '🏑', 'passive',    'CLASSWORK_XP_200',  { max_charges: 5, req_level: 27 }),
  art('leg_dragon',    'Золотой Дракон',         'Gold ×3',                                   'legendary', '🐉', 'passive',    'GOLD_MULTIPLIER_3X',{ duration_hours: 168, req_level: 28 }),
  art('leg_aegis',     'Непробиваемая Эгида',    'Все ошибки блокированы',                     'legendary', '🛡️', 'passive',    'BLOCK_ALL_MISTAKES',{ max_charges: 3, req_level: 29 }),
  art('leg_elixir',    'Эликсир Гения',          'Мгновенный уровень',                        'legendary', '🧪', 'consumable', 'FORCE_LEVEL_UP',    { req_level: 30 }),
  art('leg_ringall',   'Кольцо Всевластия',      '+10% XP всему классу',                      'legendary', '💍', 'passive',    'TEAM_XP_10',        { duration_hours: 168, req_level: 32 }),
  art('leg_scroll',    'Свиток Истины',           'XP за босса ×3',                            'legendary', '📜', 'passive',    'BOSS_MULTIPLIER_3X',{ max_charges: 1, req_level: 35 }),
  art('leg_star',      'Звезда Академии',        'Бесконечный стрик',                         'legendary', '⭐', 'passive',    'INFINITE_STREAK',   { duration_hours: 720, req_level: 40 }),
  ...
  // ── 🔥 FIRE SEASON (25) ──
  art('fire_coals',            'Угли Мотивации',     '+10 HP',                          'common', '🔥', 'consumable', 'FIRE_HP_10',           { season_tag: 'fire' }),
  art('fire_spark',            'Искра Гения',        '+50 XP',                          'common', '✨', 'consumable', 'FIRE_XP_50',           { season_tag: 'fire' }),
  art('fire_phoenix_extract',  'Экстракт Феникса',   'Полное восстановление HP',        'common', '🧪', 'consumable', 'FIRE_FULL_HP',         { season_tag: 'fire' }),
  art('fire_dwarven_ingot',    'Слиток Гномов',      '+100 Gold',                       'common', '🪙', 'consumable', 'FIRE_GOLD_100',        { season_tag: 'fire' }),
  art('fire_magma_vial',       'Флакон Магмы',       '+200 XP',                         'common', '🧪', 'consumable', 'FIRE_XP_200',          { season_tag: 'fire' }),
  art('fire_coal_stone',       'Угольный Камень',    '+500 урона боссу',                'common', '🪨', 'consumable', 'FIRE_BOSS_500',        { season_tag: 'fire' }),
  art('fire_ashes',            'Пепел Предков',      '+50 сезонного XP',                'common', '🏺', 'consumable', 'FIRE_SEASON_XP_50',    { season_tag: 'fire' }),
  art('fire_volcano_tear',     'Слеза Вулкана',      'Случайное кол-во Gold',           'common', '💧', 'consumable', 'FIRE_RANDOM_GOLD',     { season_tag: 'fire' }),
  art('fire_hellfire_elixir',  'Эликсир Пекла',      'Дарует бонус +30% к Опыту и Золоту на один квест', 'common', '⚗️', 'consumable', 'FIRE_COMBO',           { season_tag: 'fire' }),
  art('fire_ring',             'Кольцо Огня',        'Gold +10% постоянно',             'common', '💍', 'passive',    'FIRE_GOLD_MULT_10',    { season_tag: 'fire' }),
  art('fire_berserker',        'Зелье Берсерка',     'Случайный урон боссу',            'common', '🧪', 'consumable', 'FIRE_BOSS_RANDOM',     { season_tag: 'fire' }),
  art('fire_lava_amulet',      'Лавовый Амулет',     '-1 урона (постоянно)',            'common', '📿', 'passive',    'FIRE_DMG_REDUCE_1',    { season_tag: 'fire' }),
  art('fire_dragon_scale',     'Драконья Чешуйка',   'Защита стрика',                   'common', '🐉', 'passive',    'FIRE_STREAK_SHIELD',   { max_charges: 1, season_tag: 'fire' }),
  art('fire_phoenix_feather',  'Огненное Перо',      'Авто-воскрешение (50 HP)',         'common', '🪶', 'passive',    'FIRE_AUTO_RESURRECT',  { max_charges: 1, season_tag: 'fire' }),
  art('fire_banner',           'Багровое Знамя',     'Косметика',                       'common', '🏴', 'passive',    'COSMETIC',             { season_tag: 'fire' }),
  art('fire_inquisitor',       'Шлем Инквизитора',   'Косметика',                       'common', '⛑️', 'passive',    'COSMETIC',             { season_tag: 'fire' }),
  art('fire_eye_ifrit',        'Око Ифрита',         'Косметика',                       'common', '👁️', 'passive',    'COSMETIC',             { season_tag: 'fire' }),
  art('fire_flaming_sword',    'Пламенный Меч',      'Косметика',                       'common', '⚔️', 'passive',    'COSMETIC',             { season_tag: 'fire' }),
  art('fire_golden_torch',     'Золотой Факел',      'Косметика',                       'common', '🔦', 'passive',    'COSMETIC',             { season_tag: 'fire' }),
  art('fire_meteor_scroll',    'Метеоритный Дождь',  '-3000 HP боссу',                  'common', '☄️', 'consumable', 'FIRE_CLASS_BOSS_3000', { season_tag: 'fire' }),
  art('fire_hearth_scroll',    'Тепло Очага',        '+20 HP всему классу',             'common', '🏠', 'consumable', 'FIRE_CLASS_HP_20',     { season_tag: 'fire' }),
  art('fire_boom',             'Огненный Бум',       '+100 XP всему классу',            'common', '💥', 'consumable', 'FIRE_CLASS_XP_100',    { season_tag: 'fire' }),
  art('fire_dragon_hoard',     'Драконий Клад',      '+50 Gold всему классу',           'common', '💰', 'consumable', 'FIRE_CLASS_GOLD_50',   { season_tag: 'fire' }),
  art('fire_friendship_match', 'Спичка Дружбы',      '+50 XP случайному ученику',       'common', '🔥', 'consumable', 'FIRE_RANDOM_STUDENT',  { season_tag: 'fire' }),
  art('fire_dragon_claw',      'Коготь Дракона',     '+20% урона боссу',                'common', '🐲', 'passive',    'FIRE_BOSS_MULT_20',    { season_tag: 'fire' }),
  art('fire_team_boss_potion', 'Огненный Натиск',    '+20% урона боссу (весь класс)', 'rare',   '🔥', 'passive',    'TEAM_BOSS_20',         { duration_hours: 24, season_tag: 'fire' }),
];
