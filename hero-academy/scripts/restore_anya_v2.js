...
(async () => {
  ...
  // Wipe current to clear the mess of stacked items
  await supabase.from('hero_artifacts').delete().eq('hero_id', hero.id);
  
  const { data: boxes } = await supabase.from('artifacts').select('*').in('name', [
    'Обычный Сундук', 'Редкий Сундук', 'Эпический Сундук', 'Легендарный Сундук', 'Ученическое Перо'
  ]);
  
  for (const b of boxes) {
    // Insert 5 separate rows
    for(let i=0; i<5; i++) {
  ...
  console.log('Restored Anya inventory correctly!');
})();
